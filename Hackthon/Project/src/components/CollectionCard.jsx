...
const CollectionCard = ({ item }) => {
  ...
  return (
    <div className="bg-white rounded-xl overflow-hidden relative xl:w-[18vw] max-xl:w-[25vw] max-lg:w-[30vw] max-md:w-[40vw] h-80 max-sm:w-[39vw] max-sm:h-60">
      ...
      >
        <h2 className="font-semibold h-14 overflow-hidden sm:text-lg max-sm:text-sm capitalize">
          {item.title}
        </h2>
        <button
          onClick={() => {
            removedCollection(item);
          }}
className="bg-purple-600 cursor-pointer hover:bg-purple-400  transaction-active duration-300 hover:font-semibold active:scale-90 px-2 sm:py-0 py-1 sm:text-lg max-sm:text-sm rounded"        >
          Remove
        </button>
      </div>
    </div>
